...
public class MyGridView extends LinearLayout {
	...
	private ImageView createImageView(int position, final boolean isMultiImage) {
		...
		if (isMultiImage) {
			...
			Bitmap bitmap;
			if (UIUtils.isEmpty(photoInfo.path)) {
				bitmap = BitmapFactory.decodeResource(getResources(), photoInfo.url);
			} else {
				bitmap = BitmapFactory.decodeFile(photoInfo.path);
			}
			...
			if (width > height) {
				width = MAX_WIDTH * 2 / 3;
				height = (int) ((MAX_WIDTH * 2 / 3) / scale);
			} else {
				width = MAX_WIDTH * 2 / 3;
				if (scale < 1) {
					height = (int) ((MAX_WIDTH * 2 / 3) / scale);
