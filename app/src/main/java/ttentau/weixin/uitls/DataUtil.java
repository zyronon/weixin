...
public class DataUtil {
	public static ArrayList<TestFriendInfo> getListInfo(Context context) {
		...
		tfi.setName("哈哈哈傻逼");
		tfi.setType(MyCircleItem.TYPE_NORMAL);
		tfi.setContent(
		...
		tfi.setPraiseName("|~0~666~傻逼啊你SB~-1|~1~777~哎哟我草6666~-1|~2~888~你笑啥子~-1|~3~999~现在都3点钟了哎~-1");
		tfi.setCommentCount(1);
		tfi.setCommetnContent("|~0~0~哥哥~-1~你干哈啊~1~弟弟~-1|~1~0~哥哥~-1~你干哈啊|~2~0~哥哥~-1~你是不是傻逼？|~3~5~哥哥得到~-1~你是不是傻逼？");
		tfi.setDate("2017|3|10|2|46");
		tfi.setCompareData(8);

		TestFriendInfo tfi1 = new TestFriendInfo();
		tfi1.setType(MyCircleItem.TYPE_NORMAL);
		tfi1.setId("q1");
		tfi1.setIsMy(1);
		...
		tfi4.setCompareData(4);
		tfi4.setType(MyCircleItem.TYPE_NORMAL);
		...
		tfi5.setCompareData(7);
		tfi5.setType(MyCircleItem.TYPE_NORMAL);

		TestFriendInfo tfi6 = new TestFriendInfo();
		tfi6.setType(MyCircleItem.TYPE_URL);
		tfi6.setId("6");
