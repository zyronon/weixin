...
import ttentau.friendcricle.WeixinApplication;
import ttentau.friendcricle.bean.CommentItem;
import ttentau.friendcricle.utils.UrlUtils;
...
public class CommentListView extends LinearLayout {
    ...
    private View getView(final int position){
        ...
        builder.append(setClickableSpan(name, bean.getUser().getId(),bean.getUser().getPhoto()));

        if (!TextUtils.isEmpty(toReplyName)) {

            builder.append(" 回复 ");
            builder.append(setClickableSpan(toReplyName, bean.getToReplyUser().getId(),bean.getToReplyUser().getPhoto()));
        }
        builder.append(": ");
        //转换表情字符
    ...
    @NonNull
    private SpannableString setClickableSpan(final String name, final String userId, final String photo) {
        SpannableString subjectSpanText = new SpannableString(name);
        subjectSpanText.setSpan(new SpannableClickable(itemColor){
                                    @Override
                                    public void onClick(View widget) {
                                        Toast.makeText(WeixinApplication.getContext(),"name>>>>"+name+"<<<userId>>>"+ userId + " <<<<photo = " + photo, Toast.LENGTH_SHORT).show();
                                    }
